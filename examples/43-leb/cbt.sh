...
cbt__SetBitValue(const int cbtID, uint bufferID, uint bitID, uint bitValue)
{
    ...
    atomicAnd(u_CbtBuffer[bufferID], bitMask);
    atomicOr(u_CbtBuffer[bufferID], bitValue << bitID);
}
...
cbt__BitFieldInsert(
    ...
    atomicAnd(u_CbtBuffer[bufferID], bitMask);
    atomicOr(u_CbtBuffer[bufferID], bitData << bitOffset);
}
...
cbt_Node cbt__CeilNode(const int cbtID, in const cbt_Node node)
{
    ...
    return cbt__CeilNode_Fast(cbtID, node);
}
