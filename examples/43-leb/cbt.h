...
#ifndef CBT_MALLOC
#    include <stdlib.h>
#    define CBT_MALLOC(x) (malloc((size_t)x))
...
#ifndef CBT_MEMCPY
#    include <string.h>
#    define CBT_MEMCPY(dst, src, num) memcpy(dst, src, (size_t)num)
#endif
...
CBTDEF bool cbt_IsCeilNode(const cbt_Tree *tree, const cbt_Node node)
{
    return (node.depth == (uint64_t) cbt_MaxDepth(tree));
}
...
CBT_PARALLEL_FOR
    for (int64_t bufferID = bufferMinID; bufferID < bufferMaxID; ++bufferID) {
...
CBT_BARRIER
    ...
    while (--depth >= 0) {
        uint64_t min = 1ULL << depth;
        uint64_t max = 2ULL << depth;

CBT_PARALLEL_FOR
        for (uint64_t j = min; j < max; ++j) {
